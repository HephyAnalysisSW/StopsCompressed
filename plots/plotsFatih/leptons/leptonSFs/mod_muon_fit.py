...
fpout = open(
    "/groups/hephy/cms/fatih.okcu/StopsCompressed/results/%s/fits/final/mu_%s_%s.params" % (datatag, mode, stage), "w")
sout = "\t".join(pout)
fpout.write(sout + "\n")

fin = TFile(
    "/groups/hephy/cms/fatih.okcu/StopsCompressed/results/%s/hists/mu_hists_%s_%s.root" % (datatag, mode, stage))
print fin
...
fout = TFile("/groups/hephy/cms/fatih.okcu/StopsCompressed/results/%s/fits/muon_result_%s_%s_%s.root" % (datatag, mode, stage, etabin), "recreate")
...
for ipt in range(len(binning) - 1):
    ...
    savedir = "/groups/hephy/cms/fatih.okcu/www/StopsCompressed/TnP/%s/fits/%s/%s" % (datatag, mode, stage)
    makeDir(savedir)
