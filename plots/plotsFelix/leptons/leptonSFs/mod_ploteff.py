...
f = TFile("/groups/hephy/cms/felix.lang/StopsCompressed/results/%s/fits/%s_result_MC_%s%s.root"%(datatag,flavor,stage,suffix))
effcntZMC = TEfficiency(f.Get("effcnt"))
efffitZMC = TEfficiency(f.Get("efffit"))
f.Close()
f = TFile("/groups/hephy/cms/felix.lang/StopsCompressed/results/%s/fits/%s_result_Data_%s%s.root"%(datatag,flavor,stage,suffix))
...
savedir = "/groups/hephy/cms/felix.lang/www/StopsCompressed/TnP/%s/finalplots/%s"%(datatag,stage)
makeDir(savedir)
makeDir("/groups/hephy/cms/felix.lang/StopsCompressed/results/%s/finalplots"%datatag)

c1.SaveAs("/groups/hephy/cms/felix.lang/StopsCompressed/results/%s/finalplots/%s_eff_%s_%s.png"%(datatag,flavor,stage,etabin))
c1.SaveAs("%s/%s_eff_%s_%s.png"%(savedir,flavor,stage,etabin))
fout = TFile("/groups/hephy/cms/felix.lang/StopsCompressed/results/%s/finalplots/%s_eff_%s_%s.root"%(datatag,flavor,stage,etabin),"RECREATE")
c1c = c1.Clone()
c1c.Write()
fout.Close()
...
c2.SaveAs("/groups/hephy/cms/felix.lang/StopsCompressed/results/%s/finalplots/%s_SF_%s_%s.png"%(datatag,flavor,stage,etabin))
c2.SaveAs("%s/%s_SF_%s_%s.png"%(savedir,flavor,stage,etabin))
fout = TFile("/groups/hephy/cms/felix.lang/StopsCompressed/results/%s/finalplots/%s_SF_%s_%s.root"%(datatag,flavor,stage,etabin),"RECREATE")
c2c = c2.Clone()
c2c.Write()
fout.Close()
