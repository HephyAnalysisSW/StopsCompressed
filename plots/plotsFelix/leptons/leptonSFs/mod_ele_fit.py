...
makeDir("/groups/hephy/cms/felix.lang/StopsCompressed/results/%s/fits/noIso"%datatag)

pout = ["lowedge","pthigh","mean","sigma","alpha","n","sigma2","gaus1f","a","signal","bkg"]

fpout = open("/groups/hephy/cms/felix.lang/StopsCompressed/results/%s/fits/noIso/el_%s_%s.params"%(datatag,mode,stage),"w")
sout = "\t".join(pout)
fpout.write(sout+"\n")
...
fin = TFile("/groups/hephy/cms/felix.lang/StopsCompressed/results/%s/hists/noIso/ele_histos_%s_%s.root"%(datatag,mode,stage))
print fin
...
fout = TFile("/groups/hephy/cms/felix.lang/StopsCompressed/results/%s/fits/noIso/ele_result_%s_%s_%s.root"%(datatag,mode,stage,etabin),"recreate")
...
#for ipt in [1]:
    ...
    savedir = "/groups/hephy/cms/felix.lang/www/StopsCompressed/TnP/%s/fits/noIso/%s/%s"%(datatag,mode,stage)
    makeDir(savedir)
